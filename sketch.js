...
function preGame(){
  if(df === "Start"){
    ...
    //direct to easy level
    if(keyCode===69 && df === "Start"){
      df = "Easy"
    }
    //direct to medium level
    if(keyCode===77 && df === "Start"){
      df = "Medium"
    }

    //direct to hard level
...
function allGame(){
  if(df === "Easy" || df === "Medium" || df === "Hard"){
    ...
    text("Drag your mouse back and shoot the ball towards the cup",20,60);
    text("Take as many tries as you need by pressing R to retry",20,90);
    text("Once your done, Hit space to go back to the home screen to do a different level",20,120);
    ground.display();
    ground1.display();
    ground2.display();
    ground3.display();
